...
$('#home').live( 'pagebeforecreate',function(event)
{
	...
	$ul.append('<li class="pro-range"><a href="product-range.html"><h3>Product range</h3><p>'+all_data.data[0].content+'</p></a></li>');
	$ul.append('<li class="guide"><a href="guide.html"><h3>Selection guide</h3><p>'+all_data.data[1].content+'</p></a></li>');
	$ul.append('<li class="sort"><a href="sort-by.html"><h3>Sort by features</h3><p>'+all_data.data[2].content+'</p></a></li>');

	if(all_data.campaign.length > 0)
		$ul.append('<li class="local"><a href="campaigns.html"><h3>Local campaigns<span class="ui-li-count" id="campaigns_count">'+all_data.campaign.length+'</span></h3><p>'+all_data.data[3].content+'</p></a></li>');
	else
...
$('#product-range').live( 'pagebeforecreate',function(event)
{
	...
	$.each(arr , function(key , current)
	{
		...
		$span = $('<span />').addClass('model-no').text(current.series);
...
$('#product-a').live( 'pagebeforecreate',function(event)
{
	...
	{	
		...
		$count = $('<em class="count" />').text(index);
		$h5 = $('<h5/>').text(current.question);
...
$('#recommend').live( 'pagebeforecreate',function(event)
{
	...
	$.each(arr , function(key , current)
	{
		...
		$count = $('<em class="count" />').text(key+1);
		$h2 = '<h2>'+current.title+'</h2>' ;
		$img = '<img src="'+ convert_to_grey(current.image[0]) +'" alt="" class="ui-btn-left thumbnail" />' ; 
		$ul = $('<ul class="product-list" />');
		$ul_data = '';
...
$('#where-to-buy').live( 'pagebeforecreate',function(event)
{
	...
	$.each(arr , function(key , current)
	{
		...
		$h3 = $('<h3 />').text(current.name);
		$desc = $('<span class="description" />').text(current.address);
		$country = $('<span class="country" />').text(current.country);
...
$('#guide').live( 'pagebeforecreate',function(event)
{
	...
	{
		var id = current.id ; 
		$('#question'+id).text(current.question);
...
$('#social').live( 'pagebeforecreate',function(event)
{
	...
	{
		$a = $('<a target="_blank" />').attr('href' , urls[key]).text(value);
		$li = $('<li />').append($a);
		$markup.append($li);
	});
